...
| Password and account recovery | Password policy, history, expiry, email verification, reset tokens, and configurable re-authentication age for email changes | Password credentials and required actions configured per realm; email updates can require re-authentication based on maximum authentication age | Implemented / Partial | Preserve current flows; align required-action metadata with realm settings. |
