...
- The public login page may render only the enabled social providers returned by `/api/auth/social-providers` (`google`, `github`, `linkedin`, or `microsoft`). Providers without saved credentials remain visibly disabled until configured. Start each configured flow through a full-page browser redirect to `/oauth2/authorization/{registrationId}`; do not route this backend endpoint through the SPA or implement provider token exchange in the browser.
...
- The Account Console security page may list enabled provider configuration and link status from `/api/account/social-links`; unconfigured providers remain visible but disabled until an administrator saves credentials. Starting a link must use the full-page `/account/social-links/{registrationId}/start` redirect so the server can require the existing local session and complete the OAuth callback. Show `Bağla` for configured unlinked providers and a confirmed `Kaldır` action for linked providers through `DELETE /api/account/social-links/{provider}`. Disable duplicate link/unlink clicks, show inline progress spinners, and keep provider subjects server-side.
