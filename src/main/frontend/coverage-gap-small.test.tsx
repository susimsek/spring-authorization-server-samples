...
jest.mock("@/routing/navigation", () => ({ useParams: () => mockUseParams(), useRouter: jest.fn() }));
jest.mock("react-router-dom", () => ({
  Navigate: ({ to }: { to: string }) => <a href={to}>navigate</a>,
}));
jest.mock("./components/admin/RoleDetail", () => ({ RoleDetail: ({ name, tab }: { name: string; tab: string }) => <div>{name}:{tab}</div> }));
jest.mock("@/components/auth/ConsoleAuthorizationCallback", () => ({
  ConsoleAuthorizationCallback: ({ errorMessage }: { errorMessage: string }) => <div>{errorMessage}</div>,
}));
...
jest.mock("@/lib/authenticated-api-client", () => ({
  createAuthenticatedApiClient: () => ({ request: (...args: unknown[]) => mockRequest(...args), registerTokenHandlers: jest.fn() }),
}));
...
describe("small uncovered frontend modules", () => {
  ...
  it("routes valid roles and redirects invalid role sections", () => {
    ...
    expect(invalid.container.querySelector("a")).toHaveAttribute("href", "/admin/roles/ROLE_ADMIN/details");
  });
  ...
  it("normalizes account API responses and errors", async () => {
    ...
    await expect(requestAccount("token", { url: "/api/account" })).resolves.toEqual({ username: "admin" });

    mockRequest.mockResolvedValueOnce({ status: 400, data: { detail: "bad" } });
    await expect(requestAccount("token", { url: "/api/account" })).rejects.toEqual({ status: 400, data: { detail: "bad" } });

    mockRequest.mockRejectedValueOnce(new Error("network"));
    await expect(requestAccount("token", { url: "/api/account" })).rejects.toEqual({ status: 0, data: { message: "network" } });

    mockRequest.mockRejectedValueOnce("unknown");
    await expect(requestAccount("token", { url: "/api/account" })).rejects.toEqual({ status: 0, data: { message: "Account request failed" } });
  });
  ...
  it("copies, downloads, and prints recovery codes", async () => {
    ...
    Object.assign(URL, { createObjectURL: jest.fn().mockReturnValue("blob:codes"), revokeObjectURL: jest.fn() });
    Object.assign(window, { print: jest.fn() });
    render(<RecoveryCodesActions codes={["1111", "2222"]} labels={{ copy: "Copy", copied: "Copied", download: "Download", print: "Print" }} />);
    ...
    render(<RecoveryCodesActions codes={["3333"]} labels={{ copy: "Copy", copied: "Copied", download: "Download", print: "Print" }} />);
  ...
  it("toggles TOTP setup between QR and manual details", () => {
    render(
      <TotpSetupDetails
        setup={{ secret: "SECRET", qrCode: "data:image/png;base64,QR", algorithm: "SHA1", digits: 6, periodSeconds: 30 }}
        copy={{ qrTitle: "QR code", unableToScan: "Unable to scan", scanBarcode: "Scan barcode", secret: "Secret", type: "Type", typeTotp: "TOTP", algorithm: "Algorithm", digits: "Digits", period: "Period" }}
      />,
    );
