...
public record AdminLoginSettingsRequestDTO(
        ...
        @Schema(description = "MFA verification validity in seconds.", minimum = "1") @Min(1)
                int mfaVerificationTimeoutSeconds,
        @Schema(description = "Maximum accepted password length.", minimum = "1") @Min(1) @Max(128)
                int passwordMaximumLength,
        @Schema(description = "Minimum uppercase characters in a password.", minimum = "0") @Min(0)
                int passwordMinimumUppercase,
        @Schema(description = "Minimum lowercase characters in a password.", minimum = "0") @Min(0)
                int passwordMinimumLowercase,
        @Schema(description = "Minimum digits in a password.", minimum = "0") @Min(0)
                int passwordMinimumDigits,
        @Schema(description = "Minimum special characters in a password.", minimum = "0") @Min(0)
        ...
        @Schema(description = "Comma-separated common passwords rejected by policy.") @NotBlank
                String passwordCommonPasswords,
        @Schema(description = "Quick-login interval in milliseconds.", minimum = "0") @Min(0)
                int bruteForceQuickLoginWindowMillis,
        @Schema(description = "Minimum quick-login lock wait in seconds.", minimum = "0") @Min(0)
                int bruteForceMinimumQuickLoginWaitSeconds,
        @Schema(description = "Additional lock wait per failure in seconds.", minimum = "0") @Min(0)
                int bruteForceWaitIncrementSeconds,
        @Schema(description = "Maximum brute-force lock wait in seconds.", minimum = "0") @Min(0)
                int bruteForceMaxWaitSeconds,
        @Schema(description = "Failure counter reset period in seconds.", minimum = "0") @Min(0)
        ...
        @Schema(description = "Allowed login requests per IP per minute.", minimum = "1") @Min(1)
